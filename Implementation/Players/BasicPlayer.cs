...
namespace Implementation.Players
{
    internal class BasicPlayer : DisposableBase, IPlayer, IEventProvider, IReferenceCount, INativePointer
    {
        ...
        public virtual void Play()
        {
            ...
            /*
            libvlc_state_t result;
            int Count = 10;
            do
            {
                System.Threading.Thread.Sleep(50);
                result = LibVlcMethods.libvlc_media_player_get_state(m_hMediaPlayer);
                Count--;
                if (Count <= 0)
                    break;
            } while (result != libvlc_state_t.libvlc_Playing);

            if(result!= libvlc_state_t.libvlc_Playing)
                LibVlcMethods.libvlc_media_player_play(m_hMediaPlayer);
            */
        }
        ...
        public void Stop()
        {
            var thread = new System.Threading.Thread(delegate () { LibVlcMethods.libvlc_media_player_stop(m_hMediaPlayer); });
            thread.Start();
            thread.Join();
